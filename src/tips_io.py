...
def _hex_to_int(hex_colour: Optional[str]) -> Optional[int]:
    ...
    if not hex_colour:
        return None
    return Colours.HEX_TO_INT.get(hex_colour.lower())
...
def _get_html(eml_path: pathlib.Path) -> bytes:
    ...
    try:
        ...
        raise ParsingError(f"No text/html part found in {eml_path}")
    except Exception as e:
        logger.error("Failed to read HTML from %s: %s", eml_path, e)
        raise ParsingError(f"HTML extraction failed: {e}")


def _parse_exchange(soup: BeautifulSoup, eml_path: pathlib.Path) -> Dict[str, Any]:
    ...
    if date_tag:
        try:
            ...
            logger.warning("Failed to parse tip date from %s: %s", eml_path, e)
    ...
    spans_16 = [
        ...
        if FontSizes.EXCHANGE_SUMMARY in s.get("style", "")
    ]
    for s in spans_16:
...
def _parse_original_format(card_td, tip_n: int) -> Dict[str, Any]:
    """Parse a tip card in the original format (tips 1-3)."""
    ...
    for p in card_td.find_all("p", style=True):
        if FontSizes.WIN_PROB in p.get("style", ""):
    ...
    for p in card_td.find_all("p", style=True):
        if FontSizes.TRADE_LEVELS in p.get("style", ""):
    ...
    for p in card_td.find_all("p", style=True):
        if FontSizes.EXP_REWARD in p.get("style", ""):
            sib = p.find_next_sibling("p")
            label = _clean(sib.get_text()) if sib else ""
            txt = _clean(p.get_text())
    ...
    score_ps = [
        ...
        if FontSizes.SCORE_BARS in p.get("style", "") and "700" in p.get("style", "")
    ]
    if len(score_ps) >= 4:
        def _score_val(p):
            txt = _clean(p.get_text())
...
def _parse_new_format(card_td, tip_n: int) -> Dict[str, Any]:
    """Parse a tip card in the new format (tips 4+)."""
    ticker_a = card_td.find("a", href=re.compile(r'stockdataanalytics\.com/news/'))
    ticker = _clean(ticker_a.get_text()) if ticker_a else None
    url = ticker_a["href"] if ticker_a else None
    code = f"{ticker}.US" if ticker else None
...
def _parse_tip_card(card_td, tip_n: int) -> Dict[str, Any]:
    """Parse one tip card <td> into a dict, delegating to format-specific parsers."""
    if tip_n <= 3:
        return _parse_original_format(card_td, tip_n)
    else:
...
def parse_tip_email(eml_path: pathlib.Path, soup_folder=None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    all_links = soup.find_all("a", href=re.compile(r'stockdataanalytics\.com/news/'))
    seen = set()
    tip_rows = []
    for a in all_links:
...
def parse_tip_emails(eml_paths: List[pathlib.Path]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for path in eml_paths:
        try:
            ...
            continue
    exchange_df = pd.concat(exc_frames, ignore_index=True) if exc_frames else pd.DataFrame()
    tips_df = pd.concat(tip_frames, ignore_index=True) if tip_frames else pd.DataFrame()
    return exchange_df, tips_df
...
def tips_exchange2sqlite(
    ...
    """Write exchange_df and tips_df to SQLite using bulk inserts.
    ...
    try:
        ...
        conn.execute(SQL.DDL_TIP_EXCHANGE.format(tablename=exchange_tablename))
        conn.execute(SQL.DDL_TIP_DETAILS.format(tablename=tips_tablename))
        ...
        logger.error("SQLite error in tips_exchange2sqlite: %s", e)
...
def tips_sqlite2pandas(
    ...
    try:
        ...
        logger.error("SQLite error in tips_sqlite2pandas: %s", e)
    ...
    if exchange_df.empty:
        exchange_df = pd.DataFrame(columns=[
            "exchange", "tip_date", "market_state", "week_pct", "week_colour",
            "month_pct", "month_colour", "volatility", "regime_score", "regime_colour"
        ])
    if tips_df.empty:
        tips_df = pd.DataFrame(columns=[
            "exchange", "tip_date", "tip_n", "code", "win_probability", "sector", "name",
            "entry_zone_low", "entry_zone_high", "target", "stop",
            "expected_reward", "expected_risk", "holding_period_low", "holding_period_high",
            "url", "pattern_quality_number", "pattern_quality_colour", "setup_number",
            "setup_colour", "risk_reward_number", "risk_reward_colour", "context_number",
            "context_colour"
        ])
    ...
    tip_int_cols = [
        "win_probability", "holding_period_low", "holding_period_high",
        "pattern_quality_colour", "setup_colour", "risk_reward_colour", "context_colour"
    ]
