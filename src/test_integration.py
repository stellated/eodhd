...
test_integration.py
--------------------
Integration tests for email_downloader.py and tips_io.py using real sample_emails from ../sample_emails/.
...
class TestRealEmails:
    """Integration tests using real sample_emails from ../data/sample_emails/."""

    @pytest.mark.skipif(not Path("../data/sample_emails").exists(), reason="Sample sample_emails directory not found")
    def test_parse_real_emails(self):
        """Test parsing all real sample_emails in ../data/sample_emails/."""
    ...
    def test_parse_all_real_emails(self):
        """Test parse_tip_emails on all real sample_emails."""
        emails_dir = Path(__file__).parent.parent / "data" / "sample_emails"
        eml_files = list(emails_dir.glob("*.eml"))
        assert len(eml_files) > 0, f"No .eml files found in {emails_dir}"
    ...
    def test_full_workflow_with_real_emails(self, tmp_path):
        """Test the full workflow: parse real sample_emails -> write to SQLite -> read back."""
        emails_dir = Path(__file__).parent.parent / "data" / "sample_emails"
        eml_files = list(emails_dir.glob("*.eml"))
        assert len(eml_files) > 0, f"No .eml files found in {emails_dir}"

        # Parse all real sample_emails
        exchange_df, tips_df = parse_tip_emails(eml_files)
